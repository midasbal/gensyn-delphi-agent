import { listOpenMarkets, listMarketsByStatus, type MarketStatus } from "../sdk/client.js";
...
export async function fetchMarketsByStatus(status: MarketStatus, opts?: { limit?: number }): Promise<NormalizedMarket[]> {
  const markets = await listMarketsByStatus(status, opts);
  return markets.map(normalizeMarket);
}
